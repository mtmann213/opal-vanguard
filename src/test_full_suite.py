...
def run_logic_check(name, overrides):
    ...
    pkt.handle_msg(
        pmt.cons(pmt.make_dict(), pmt.init_u8vector(len(payload), list(payload)))
    )
    ...
    try:
        with open("src/usrp_transceiver.py", "r") as f:
            ...
            if "mult_len" in content and "sizeof_gr_complex" in content:
                print(
                    "\033[92m[PASS]\033[0m Native C++ scaling found with correct sample-domain sizing."
                )
            else:
                print(
                    "\033[91m[FAIL]\033[0m High-speed scaling block missing or misconfigured."
                )
                timing_ok = False
    except Exception as e:
        print(f"[ERROR] PHY timing check failed: {e}")
        timing_ok = False
    ...
    try:
        with open("src/packetizer.py", "r") as f:
            if "self.sync_hex = p_cfg.get('syncword'" in f.read():
                print(
                    "\033[92m[PASS]\033[0m Packetizer correctly implements dynamic syncwords."
                )
            else:
                print(
                    "\033[91m[FAIL]\033[0m Packetizer is missing dynamic syncword support."
                )
                config_ok = False
    except Exception as e:
        print(f"[ERROR] Config parity check failed: {e}")
        config_ok = False
    except:
        config_ok = False
