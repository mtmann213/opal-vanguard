#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Opal Vanguard - Headless USRP Node (v15.8.15 Restoration)
...
class OpalVanguardUSRPHeadless(gr.top_block):
    def __init__(
        ...
        print(
            f"[{self.role}] Setting up modulation ({p_cfg.get('modulation', 'GFSK')})..."
        )
        self.pdu_src = blocks.message_strobe(
            pmt.cons(
                pmt.make_dict(),
                pmt.init_u8vector(len("MISSION DATA"), list("MISSION DATA".encode())),
            ),
            3000,
        )
        ...
        if mod_type == "DBPSK":
            ...
            freq_dev = p_cfg.get("freq_dev", 25000)
        ...
        self.hop_ctrl = tod_hop_generator(
            key=bytes.fromhex(hcfg.get("aes_key", "00" * 32)),
            num_channels=hcfg.get("num_channels", 50),
            center_freq=self.center_freq,
            channel_spacing=hcfg.get("channel_spacing", 150000),
            dwell_ms=hcfg.get("dwell_time_ms", 500),
        )
                ...
                try:
                    ...
                    if f > 0 and f != self.last_f:
                        if t > (time.time() + 0.010):
                            ...
                            self.parent.usrp_sink.set_command_time(cmd_time, 0)
                            self.parent.usrp_source.set_command_time(cmd_time, 0)
                            self.parent.usrp_sink.set_center_freq(f, 0)
                            self.parent.usrp_source.set_center_freq(f, 0)
                            self.parent.usrp_sink.clear_command_time(0)
                            self.parent.usrp_source.clear_command_time(0)
                        else:
                            self.parent.usrp_sink.set_center_freq(f, 0)
                            self.parent.usrp_source.set_center_freq(f, 0)
                        self.last_f = f
                except Exception as e:
                    pass  # Silent fail on frequency handler errors
        ...
        class DiagPrinter(gr.basic_block):
            def __init__(self, role):
                gr.basic_block.__init__(self, "DiagPrinter", None, None)
                self.role = role
                self.message_port_register_in(pmt.intern("msg"))
                self.set_msg_handler(pmt.intern("msg"), self.handle)
            ...
            def handle(self, msg):
                try:
                    ...
                    if pmt.is_true(ok_pmt):
                        repairs = pmt.to_long(
                            pmt.dict_ref(
                                msg, pmt.intern("fec_repairs"), pmt.from_long(0)
                            )
                        )
                        conf = pmt.to_double(
                            pmt.dict_ref(
                                msg, pmt.intern("confidence"), pmt.from_double(0)
                            )
                        )
                        print(
                            f"[{self.role}] \033[92mCRC PASS\033[0m | LQI: {conf:.1f}% | FEC: {repairs}"
                        )
                except Exception as e:
                    pass  # Silent fail on diag handler errors

        self.dp = DiagPrinter(role)
        self.msg_connect((self.depkt_b, "diagnostics"), (self.dp, "msg"))

        class DataPrinter(gr.basic_block):
            def __init__(self, role):
                gr.basic_block.__init__(self, "DataPrinter", None, None)
                self.role = role
                self.message_port_register_in(pmt.intern("msg"))
                self.set_msg_handler(pmt.intern("msg"), self.handle)
        ...
        self.rx_prnt = DataPrinter(role)
        self.msg_connect((self.depkt_b, "out"), (self.rx_prnt, "msg"))

        self._hop_event = threading.Event()
        self._hop_dwell = h_cfg.get("dwell_time_ms", 1000) / 1000.0
        if h_cfg.get("enabled", True):
            self._hop_thread = threading.Thread(target=self._hop_loop, daemon=True)
            self._hop_thread.start()

    def _hop_loop(self):
        while not self._hop_event.is_set():
            self.hop_ctrl.handle_trigger(pmt.PMT_T)
            self._hop_event.wait(self._hop_dwell)

    def stop(self):
        self._hop_event.set()
        super(OpalVanguardUSRPHeadless, self).stop()
...
def main():
    ...
    tb = OpalVanguardUSRPHeadless(
        role=args.role, serial=args.serial, config_path=args.config
    )
    tb.start()
    print(f"Opal Vanguard Headless Node - {args.role} [{args.serial}] Started.")
    try:
        while True:
            time.sleep(1)
