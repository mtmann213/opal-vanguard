...
class tod_hop_generator(gr.basic_block):
    def __init__(
        ...
        gr.basic_block.__init__(
            self, name="tod_hop_generator", in_sig=None, out_sig=None
        )
        ...
        self._seed_override = None  # For session-based seed sync
    ...
    def handle_set_seed(self, msg):
        ...
        print(f"[TOD Hop] Seed override: {seed}")
    ...
    def handle_trigger(self, msg):
        ...
        if final_idx != raw_idx:
            print(
                f"\033[96m[AFH Hop] {time.strftime('%H:%M:%S')} | EVADED {raw_idx} -> Moved to {final_idx}\033[0m"
            )
        else:
        ...
        out_dict = pmt.dict_add(
            out_dict, pmt.intern("time"), pmt.from_double(epoch_start_time)
        )
