#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Opal Vanguard - Full FHSS Lab (TOD-Sync + DSSS + Config)
...
class OpalVanguardVisualDemo(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, config_path="config.yaml"):
        ...
        mode_label = Qt.QLabel(f"MODE: {hcfg['sync_mode']} | {hcfg['dwell_time_ms']}ms Dwell")
        mode_label.setStyleSheet("color: blue; font-size: 10px;")
        self.ctrl_panel.addWidget(mode_label)

        # Sliders
        self.ctrl_panel.addWidget(Qt.QLabel("<b>Channel Stress Test</b>"))
        self.noise_val = Qt.QLabel("Noise Voltage: 0.00 V"); self.ctrl_panel.addWidget(self.noise_val)
        ...
        self.freq_val = Qt.QLabel("Freq Offset: 0.0 Hz"); self.ctrl_panel.addWidget(self.freq_val)
        self.freq_slider = Qt.QSlider(Qt.Qt.Horizontal); self.freq_slider.setRange(-500, 500); self.freq_slider.valueChanged.connect(self.update_channel); self.ctrl_panel.addWidget(self.freq_slider)
        self.time_val = Qt.QLabel("Timing Offset: 1.000"); self.ctrl_panel.addWidget(self.time_val)
        self.time_slider = Qt.QSlider(Qt.Qt.Horizontal); self.time_slider.setRange(990, 1010); self.time_slider.setValue(1000); self.time_slider.valueChanged.connect(self.update_channel); self.ctrl_panel.addWidget(self.time_slider)
        self.burst_val = Qt.QLabel("Burst Jammer: OFF"); self.ctrl_panel.addWidget(self.burst_val)
        self.burst_slider = Qt.QSlider(Qt.Qt.Horizontal); self.burst_slider.setRange(0, 100); self.burst_slider.valueChanged.connect(self.update_channel); self.ctrl_panel.addWidget(self.burst_slider)
        ...
        if hcfg['sync_mode'] == "TOD":
            # In TOD mode, sessions skip handshake
        ...
        self.connect(self.channel, self.viz_throttle); self.connect(self.viz_throttle, self.snk_waterfall)
    ...
    def on_status_change(self, state_a, state_b):
        ...
        else: self.status_label.setStyleSheet("font-weight: bold; color: orange; font-size: 14px;")
...
def main():
    ...
    tb = OpalVanguardVisualDemo(); tb.start(); tb.show()
