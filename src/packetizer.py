#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Opal Vanguard - Mission-Controlled Packetizer (Alignment Build v7.4)
...
class packetizer(gr.basic_block):
    ...
    def handle_msg(self, msg):
        ...
        if self.use_fec:
            if "LINK-16" in self.fec_mode or "LEVEL_6" in self.fec_mode:
                ...
                for j in range(0, len(p_bits), 75):
                    ...
                    for s in encoded:
                        for k in range(5): f_bits.append((s >> (4-k)) & 1)
                f_bytes = []; acc = 0; bc = 0
                for b in f_bits:
                    acc = (acc << 1) | b; bc += 1
                    if bc == 8: f_bytes.append(acc); acc = 0; bc = 0
        ...
        # 4. Padding to fixed block size
        target_bytes = 320 if "LINK-16" in self.fec_mode or "LEVEL_6" in self.fec_mode else 120
        if len(packet) < target_bytes: packet = packet.ljust(target_bytes, b'\x00')
        else: packet = packet[:target_bytes]
