...
class packetizer(gr.basic_block):
    ...
    def __init__(self, config_path="mission_configs/level1_soft_link.yaml", src_id=0):
        ...
        self.use_ccsk = (
            self.cfg.get("dsss", {}).get("enabled", False)
            and self.cfg.get("dsss", {}).get("type") == "CCSK"
        )

        # Unified tactical mode flag (used by both packetizer and depacketizer)
    ...
    def handle_msg(self, msg):
        ...
        if pmt.is_dict(pmt.car(msg)):
            seq = pmt.to_long(
                pmt.dict_ref(pmt.car(msg), pmt.intern("seq"), pmt.from_long(0))
            )
            m_type = pmt.to_long(
                pmt.dict_ref(pmt.car(msg), pmt.intern("type"), pmt.from_long(0))
            )
        ...
        if self.use_comsec and self.comsec_key and m_type == 0:
            ...
            payload = nonce + enc.update(payload) + enc.finalize()
        ...
        if self.use_fec:
            ...
            for i in range(0, len(raw_block), 11):
                ...
                for k in range(0, 30, 2):
                    fec_payload += bytes([((all_e[k] << 4) | all_e[k + 1])])
        ...
        self.message_port_pub(
            pmt.intern("out"),
            pmt.cons(pmt.make_dict(), pmt.init_u8vector(len(out_bits), out_bits)),
        )
