...
class depacketizer(gr.basic_block):
    def __init__(
        ...
        gr.basic_block.__init__(
            self, name="depacketizer", in_sig=[np.uint8], out_sig=None
        )
        ...
        self.target_bits = np.array(
            [int(b) for b in format(self.sync_val, f"0{self.sync_len}b")],
            dtype=np.uint8,
        )
        ...
        self.comsec_key = (
            bytes.fromhex(l_cfg.get("comsec_key", "00" * 32))
            if self.use_comsec
            else None
        )
        self.interleaver = MatrixInterleaver(rows=l_cfg.get("interleaver_rows", 15))
        self.scrambler = Scrambler(
            mask=l_cfg.get("scrambler_mask", 0x48),
            seed=l_cfg.get("scrambler_seed", 0x7F),
        )
        self.nrzi = NRZIEncoder()
        self.ccsk = CCSKProcessor()

        dsss_cfg = self.cfg.get("dsss", {})
        self.is_tactical = (
            dsss_cfg.get("enabled", False) and dsss_cfg.get("type") == "CCSK"
        )
        ...
        self._scrambler_lock = threading.Lock()
            ...
            if self.use_fec:
                ...
                for j in range(0, len(data_block), 15):
                    ...
                    for k in range(0, 22, 2):
                        healed.append((combined[k] << 4) | combined[k + 1])
            ...
            if crc_pass:
                if not (self.ignore_self and sid == self.src_id):
                    ...
                    if self.use_comsec and self.comsec_key and m_type == 0:
                        ...
                        cipher = Cipher(
                            algorithms.AES(self.comsec_key),
                            modes.CTR(nonce),
                    ...
                    t_name = {0: "DATA", 1: "SYN", 2: "ACK", 3: "NACK"}.get(
                        m_type, "UNK"
                    )
                    print(
                        f"\033[92m[OK]\033[0m ID: {seq:03} | TYPE: {t_name} | RX: {payload}"
                    )
                    meta = pmt.make_dict()
                    meta = pmt.dict_add(meta, pmt.intern("type"), pmt.from_long(m_type))
                    self.message_port_pub(
                        pmt.intern("out"),
                        pmt.cons(meta, pmt.init_u8vector(len(payload), list(payload))),
                    )

                diag = pmt.make_dict()
                diag = pmt.dict_add(diag, pmt.intern("crc_ok"), pmt.PMT_T)
                diag = pmt.dict_add(
                    diag, pmt.intern("confidence"), pmt.from_double(confidence * 100.0)
                )
                diag = pmt.dict_add(
                    diag, pmt.intern("fec_repairs"), pmt.from_long(repairs_made)
                )
                self.message_port_pub(pmt.intern("diagnostics"), diag)
        except Exception as e:
            logger.debug(f"Block decode error (recovered by CRC): {e}")
    ...
    def general_work(self, input_items, output_items):
        ...
        if self.state == "SEARCH":
            ...
            try:
                ...
                pass  # Silent fail on sliding window edge case
            self.consume(0, max(0, n - self.sync_len + 1))
            return 0

        if self.state == "COLLECT":
            bits_per_frame = self.frame_size * 8
            chips_per_frame = (
                (bits_per_frame // 5) * 32 if self.is_tactical else bits_per_frame
            )
            ...
            if len(self.recovered_bits) >= chips_per_frame:
                raw_chips = np.array(
                    self.recovered_bits[:chips_per_frame], dtype=np.uint8
                )
                if self.is_tactical:
                    ...
                    bits_arr = (
                        ((best_symbols[:, None] & mask) > 0).astype(np.uint8).flatten()
                    )
                else:
                    bits_arr = raw_chips

                if self.use_nrzi and not self.is_tactical:
                    bits_arr = np.array(
                        self.nrzi.decode(bits_arr.tolist()), dtype=np.uint8
                    )
