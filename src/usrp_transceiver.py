...
class OpalVanguardUSRP(gr.top_block, Qt.QWidget):
    ...
    def __init__(
        ...
        self.payload_type = self.cfg.get("application_layer", {}).get(
    ...
    def setup_ui(self, role, serial, hw_cfg):
        ...
        self.info_label = Qt.QLabel(
            f"<b>MISSION: {mission_id} | NODE: {role} | SDR: {serial}</b>"
        )
        self.status_label = Qt.QLabel("Status: IDLE")
        ...
        self.conf_bar = Qt.QProgressBar()
        self.conf_bar.setRange(0, 100)
        self.health_layout.addWidget(Qt.QLabel("LQI (Signal Confidence %):"))
        self.health_layout.addWidget(self.conf_bar)

        self.status_row = Qt.QHBoxLayout()
        self.crc_led = Qt.QLabel("CRC: ---")
        self.fec_count = Qt.QLabel("FEC: 0")
        self.status_row.addWidget(self.crc_led)
        self.status_row.addWidget(self.fec_count)
        ...
        self.hw_layout = Qt.QHBoxLayout(self.hw_group)
        self.toc_layout.addWidget(self.hw_group, 2, 0, 1, 2)
        self.tx_gain_slider = Qt.QSlider(Qt.Qt.Horizontal)
        self.tx_gain_slider.setRange(0, 90)
        self.tx_gain_slider.setValue(hw_cfg.get("tx_gain", 70))
        self.rx_gain_slider = Qt.QSlider(Qt.Qt.Horizontal)
        self.rx_gain_slider.setRange(0, 90)
        self.rx_gain_slider.setValue(hw_cfg.get("rx_gain", 70))
        self.hw_layout.addWidget(Qt.QLabel("TX:"))
        self.hw_layout.addWidget(self.tx_gain_slider)
        self.hw_layout.addWidget(Qt.QLabel("RX:"))
        self.hw_layout.addWidget(self.rx_gain_slider)
        self.tx_gain_slider.valueChanged.connect(
            lambda v: self.usrp_sink.set_gain(v, 0)
        )
        self.rx_gain_slider.valueChanged.connect(
            lambda v: self.usrp_source.set_gain(v, 0)
        )
        ...
        self.feed_layout.addLayout(self.chat_layout)
    ...
    def toggle_stealth(self):
        ...
        self.stealth_btn.setStyleSheet(
            f"background-color: {'#b33' if is_stealth else '#444'}; color: white; font-weight: bold;"
        )
        if is_stealth:
    ...
    def setup_hardware(self, hw_cfg, serial):
        ...
        try:
            self.usrp_sink = uhd.usrp_sink(
                args, uhd.stream_args(cpu_format="fc32", channels=[0]), "packet_len"
            )
            self.usrp_source = uhd.usrp_source(
                args, uhd.stream_args(cpu_format="fc32", channels=[0])
            )
            ...
            print(
                "\033[93m[HINT] Your USRP is likely claimed by a zombie process.\033[0m"
            )
            print(
                "\033[93m[HINT] Run: 'sudo killall -9 python3' and power-cycle your B205mini.\033[0m"
            )
            sys.exit(1)
    ...
    def setup_dsp(self, config_path, h_cfg, p_cfg, l_cfg):
        ...
        self.session = session_manager(
            initial_seed=h_cfg.get("initial_seed", 0xACE), config_path=config_path
        )
        self.pkt_a = packetizer(config_path=config_path, src_id=sid)
        self.depkt_b = depacketizer(
            config_path=config_path, src_id=sid, ignore_self=True
        )

        if l_cfg.get("use_comsec", False):
            key = bytes.fromhex(l_cfg.get("comsec_key", "00" * 32))
            self.pkt_a.use_comsec = True
            self.pkt_a.comsec_key = key
            self.depkt_b.use_comsec = True
            self.depkt_b.comsec_key = key
        ...
        if self.payload_type == "heartbeat":
            hb_msg = pmt.cons(
                pmt.make_dict(),
                pmt.init_u8vector(
                    len(f"PING FROM {self.role}"),
                    list(f"PING FROM {self.role}".encode()),
                ),
            )
        ...
        self.mult_len = blocks.tagged_stream_multiply_length(
            gr.sizeof_gr_complex, "packet_len", sps
        )
        ...
        if mod_type in ["GFSK", "MSK", "GMSK"]:
            ...
            default_dev = (
                bit_rate / 4.0
                if mod_type in ["MSK", "GMSK"]
                else p_cfg.get("freq_dev", 25000)
            )
            ...
            self.demod_b = digital.psk_demod(
                2, samples_per_symbol=sps, differential=True
            )
        elif mod_type == "DQPSK":
            self.mod_a = digital.psk_mod(
                4, differential=True, samples_per_symbol=sps, excess_bw=0.35
            )
            self.demod_b = digital.psk_demod(
                4, differential=True, samples_per_symbol=sps, excess_bw=0.35
            )
        elif mod_type == "OFDM":
            self.mod_a = digital.ofdm_tx(
                fft_len=64, cp_len=16, packet_length_tag_key="packet_len"
            )
            self.demod_b = digital.ofdm_rx(
                fft_len=64, cp_len=16, packet_length_tag_key="packet_len"
            )
            self.unpack = blocks.packed_to_unpacked_bb(1, gr.GR_MSB_FIRST)

        self.rx_filter = filter.fir_filter_ccf(
            1, filter.firdes.low_pass(1.0, self.samp_rate, 100e3, 50e3)
        )
        self.hop_ctrl = tod_hop_generator(
            ...
            dwell_ms=h_cfg.get("dwell_time_ms", 500),
        )
    ...
    def connect_logic(self, mod_type, h_cfg):
        ...
        print(
            f"[UI] Performance Mode: {'LOW-LATENCY' if is_high_cpu else 'STANDARD'} ({1.0 / fps_delay:.1f} FPS)"
        )

        self.snk_waterfall = qtgui.waterfall_sink_c(
            fft_size,
        ...
        self.viz_panel.addWidget(
            sip.wrapinstance(self.snk_waterfall.qwidget(), Qt.QWidget)
        )
        self.connect(self.usrp_source, self.snk_waterfall)

        class UHDHandler(gr.basic_block):
            def __init__(self, usrp_src, usrp_snk):
                gr.basic_block.__init__(self, "UHDHandler", None, None)
                self.src, self.snk = usrp_src, usrp_snk
            ...
            def handle(self, msg):
                try:
                    f = pmt.to_double(
                        pmt.dict_ref(msg, pmt.intern("freq"), pmt.from_double(0))
                    )
                    t = pmt.to_double(
                        pmt.dict_ref(msg, pmt.intern("time"), pmt.from_double(0))
                    )
        ...
        if h_cfg.get("enabled", True):
            self.timer.start(h_cfg["dwell_time_ms"])

        # Poll manual_queue for chat messages (50ms interval)
        self._manual_timer = QTimer()
        self._manual_timer.timeout.connect(self._process_manual_queue)
        self._manual_timer.start(50)

    def _process_manual_queue(self):
        if self.manual_queue and hasattr(self, "session"):
            while self.manual_queue:
                msg = self.manual_queue.pop(0)
                self.session.handle_tx_request(msg)
    ...
    def on_status_msg(self, msg):
        try:
            ...
            self.status_label.setStyleSheet(
                f"color: {'green' if s == 'CONNECTED' else 'orange'}; font-weight: bold;"
            )
        except:
            pass
    ...
    def on_data_msg(self, msg):
        try:
            raw = bytes(pmt.u8vector_elements(pmt.cdr(msg))).decode(
            ...
            if raw.startswith("BFT|"):
                ...
                if len(parts) >= 4:
                    ...
                    self.target_table.setItem(
                        row, 3, Qt.QTableWidgetItem(time.strftime("%H:%M:%S"))
                    )
            else:
    ...
    def on_diag_msg(self, msg):
        try:
            conf = pmt.to_double(
                pmt.dict_ref(msg, pmt.intern("confidence"), pmt.from_double(0))
            )
            repairs = pmt.to_long(
                pmt.dict_ref(msg, pmt.intern("fec_repairs"), pmt.from_long(0))
            )
            ...
            self.crc_led.setStyleSheet(
                f"color: {'green' if ok else 'red'}; font-weight: bold;"
            )
            self.fec_count.setText(f"FEC Repairs: {repairs}")
            ts = time.strftime("%H:%M:%S")
            self.lqi_history_list.insertItem(
                0,
                f"[{ts}] LQI: {conf:.1f}% | Repairs: {repairs} | CRC: {'OK' if ok else 'FAIL'}",
            )
            if self.lqi_history_list.count() > 50:
                self.lqi_history_list.takeItem(50)
        except:
            pass
    ...
    def send_chat(self):
        ...
        if txt:
            ...
            msg = pmt.cons(
                pmt.make_dict(), pmt.init_u8vector(len(txt), list(txt.encode()))
            )
