#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Opal Vanguard - USRP B210/B205mini Transceiver (Mission Master v4.0)
    ...
    def work(self, input_items, output_items):
        if self.capturing:
            ...
            if len(self.buffer) >= 1024:
                self.capturing = False
                self.parent.save_iq_snapshot(self.buffer[:1024])
...
class OpalVanguardUSRP(gr.top_block, Qt.QWidget):
    ...
    def __init__(self, role="ALPHA", serial="", config_path="mission_configs/level1_soft_link.yaml"):
        ...
        if self.payload_type == 'chat':
            ...
            class ChatSender(gr.basic_block):
                def __init__(self):
                    gr.basic_block.__init__(self, "ChatSender", None, None); self.message_port_register_out(pmt.intern("out"))
                def send_msg(self, text):
                    p = text.encode('utf-8'); self.message_port_pub(pmt.intern("out"), pmt.cons(pmt.make_dict(), pmt.init_u8vector(len(p), list(p))))
                def work(self, i, o): return 0
            self.pdu_src = ChatSender()
            self.chat_btn.clicked.connect(self.on_chat_send); self.chat_input.returnPressed.connect(self.on_chat_send)
        elif self.payload_type == 'file':
            self.file_layout = Qt.QHBoxLayout(); self.file_path_input = Qt.QLineEdit(); self.file_send_btn = Qt.QPushButton("Send")
            self.file_layout.addWidget(self.file_path_input); self.file_layout.addWidget(self.file_send_btn); self.ctrl_panel.addLayout(self.file_layout)
            ...
            class FileSender(gr.basic_block):
                def __init__(self, parent):
                    gr.basic_block.__init__(self, "FileSender", None, None); self.parent = parent; self.message_port_register_out(pmt.intern("out"))
                    self.fid = np.random.randint(0, 65535); self.sending = False
                ...
                def _thread(self, path):
                    try:
                        ...
                        for i, c in enumerate(chunks):
                            payload = b'FTP!' + struct.pack('>HHH', self.fid, i, len(chunks)) + c
                ...
                def work(self, i, o): return 0
            self.pdu_src = FileSender(self); self.file_send_btn.clicked.connect(self.on_file_send)
        else:
        ...
        if mod_type in ["DBPSK", "DQPSK", "D8PSK"]:
            ...
            self.mod_a = digital.gmsk_mod(sps, 0.5); self.demod_b = digital.gmsk_demod(sps, 0.1, 0.5, 0.005, 0.0)
        elif mod_type == "OFDM":
            self.mod_a = digital.ofdm_tx(64, 16, "packet_len"); self.demod_b = digital.ofdm_rx(64, 16, "packet_len")
        else:
            self.mod_a = digital.gfsk_mod(sps, (2.0*np.pi*125000)/self.samp_rate, 0.35)
            self.demod_b = digital.gfsk_demod(sps, 0.1, 0.5, 0.005, 0.0)

        self.heartbeat = analog.noise_source_c(analog.GR_GAUSSIAN, 0.0001); self.add = blocks.add_vcc(1)
        if hcfg.get('sync_mode') == "TOD":
            self.hop_ctrl = tod_hop_generator(bytes.fromhex(hcfg.get('aes_key', '00'*32)), hcfg.get('num_channels', 50), self.center_freq, hcfg.get('channel_spacing', 150000), hcfg.get('dwell_time_ms', 200), hcfg.get('lookahead_ms', 0))
        else:
            self.hop_ctrl = aes_hop_generator(bytes.fromhex(hcfg.get('aes_key', '00'*32)), hcfg.get('num_channels', 50), self.center_freq, hcfg.get('channel_spacing', 150000))

        self.rx_filter = filter.fir_filter_ccf(1, filter.firdes.low_pass(1.0, self.samp_rate, 500e3, 100e3))
        ...
        self.connect(self.heartbeat, (self.add, 1)); self.connect(self.add, self.usrp_sink)
        ...
        class UHDHandler(gr.basic_block):
            def __init__(self, parent):
                ...
                self.message_port_register_in(pmt.intern("blacklist")); self.set_msg_handler(pmt.intern("blacklist"), self.handle_bl)
            def handle_bl(self, msg): pass
            def handle(self, msg):
                try:
                    if pmt.is_dict(msg):
                        ...
                        if t > 0:
                            ts = uhd.time_spec(t); self.parent.usrp_sink.set_command_time(ts); self.parent.usrp_source.set_command_time(ts)
                        self.parent.usrp_sink.set_center_freq(f); self.parent.usrp_source.set_center_freq(f)
                        if t > 0:
                            self.parent.usrp_sink.clear_command_time(); self.parent.usrp_source.clear_command_time()
                    else:
                        f = pmt.to_double(msg); self.parent.usrp_sink.set_center_freq(f); self.parent.usrp_source.set_center_freq(f)
                    self.parent.depkt_b.current_channel = int((f - self.parent.center_freq) / self.parent.cfg['hopping']['channel_spacing'] + (self.parent.cfg['hopping']['num_channels'] // 2))
                except: pass
            def work(self, i, o): return 0
            ...
            def handle(self, msg):
                try:
                    ...
                    if pmt.dict_has_key(msg, pmt.intern("crc_ok")):
                        ok = pmt.to_bool(pmt.dict_ref(msg, pmt.intern("crc_ok"), pmt.PMT_NIL)); d["crc_ok"] = ok
        ...
        self.diag_signal.connect(self.on_diag); self.data_signal.connect(lambda p: self.text_out.append(p))
            ...
            def handle(self, msg):
                try:
                    ...
                    if p.startswith(b'FTP!'):
                        fid, idx, tot = struct.unpack('>HHH', p[4:10]); dat = p[10:]
                        if fid not in self.fbuf: self.fbuf[fid] = {'c': {}, 't': tot}; self.parent.data_signal.emit(f"<b>[RX]:</b> Incoming File ({tot} chunks)")
                        if idx not in self.fbuf[fid]['c']:
                            self.fbuf[fid]['c'][idx] = dat
                            if hasattr(self.parent, 'file_progress'): self.parent.file_progress.setValue(int(len(self.fbuf[fid]['c'])/tot*100))
                            if len(self.fbuf[fid]['c']) == tot:
                                res = b''.join([self.fbuf[fid]['c'][i] for i in range(tot)])
                                with open(f"rx_{fid}.dat", 'wb') as f: f.write(res)
                                self.parent.data_signal.emit(f"<b>[RX]:</b> <span style='color:green'>Saved rx_{fid}.dat</span>"); del self.fbuf[fid]
                    else:
                        text = p.decode('utf-8', 'ignore'); print(f"\033[94m[DATA RX]: {text}\033[0m"); self.parent.data_signal.emit(f"<b>[RX]:</b> {text}")
                except: pass
            def work(self, i, o): return 0
        self.logger = ConsoleDataLogger(self); self.msg_connect((self.session, "data_out"), (self.logger, "msg"))

        self.ghost_mode = self.cfg['physical'].get('ghost_mode', False); self.ghost_timer = Qt.QTimer(); self.ghost_timer.setSingleShot(True)
        ...
        class GhostController(gr.basic_block):
            def __init__(self, parent):
                gr.basic_block.__init__(self, "GhostController", None, None); self.parent = parent; self.message_port_register_in(pmt.intern("msg")); self.set_msg_handler(pmt.intern("msg"), self.handle)
                if self.parent.ghost_mode: print("\033[90m[PHY] Ghost Mode ACTIVE\033[0m"); self.parent.usrp_sink.set_gain(0, 0)
            def handle(self, msg):
                if self.parent.ghost_mode: self.parent.usrp_sink.set_gain(self.parent.tx_gain_slider.value(), 0); self.parent.ghost_timer.start(150)
            def work(self, i, o): return 0
        self.ghost_ctrl = GhostController(self); self.msg_connect((self.pkt_a, "out"), (self.ghost_ctrl, "msg"))

        class AMCHandler(gr.basic_block):
            def __init__(self, parent):
                gr.basic_block.__init__(self, "AMCHandler", None, None); self.parent = parent; self.message_port_register_in(pmt.intern("msg")); self.set_msg_handler(pmt.intern("msg"), self.handle)
            def handle(self, msg):
                print("\033[41m*** AMC FALLBACK REBOOT ***\033[0m"); self.parent.stop(); self.parent.wait()
                os.execv(sys.executable, [sys.executable, sys.argv[0], "--role", self.parent.role, "--serial", self.parent.serial, "--config", "mission_configs/level4_stealth.yaml"])
            def work(self, i, o): return 0
    ...
    def on_diag(self, d):
        self.crc_led.setText(f"CRC: {'PASS' if d.get('crc_ok') else 'FAIL'}"); self.crc_led.setStyleSheet(f"color: {'green' if d.get('crc_ok') else 'red'};")
